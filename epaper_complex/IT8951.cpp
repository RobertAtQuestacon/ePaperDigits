...
bool IT8951_verbose = false;

uint16_t drawMode = 6;  // A2 mode
...
void EPD_Clear(uint8_t Color)
{
  ...
  // forground seems to slowly fade, but isn't cleared. Too big an area all at once
  // also kills further writing, or maybe just very slow
}
*/
...
void IT8951DisplayExample()
{
  ...
  IT8951DisplayArea(0,0, gstI80DevInfo.usPanelW, gstI80DevInfo.usPanelH, drawMode);
}
...
uint16_t blanking_areas;
uint16_t blanking_height;

uint8_t IT8951_Init(int blank_areas = 2)  // 36 worked,  6 worked
{
  blanking_areas = blank_areas;
  //bcm2835_spi_begin();
  ...
  display_width = gstI80DevInfo.usPanelW; 
  display_height = gstI80DevInfo.usPanelH;
  blanking_height = gstI80DevInfo.usPanelH/blanking_areas;
  Serial.print("W: ");
  ...
  // try this
  //blanking_areas = display_height;
  //blanking_height = 1;
  gpFrameBuf = (uint8_t*)malloc(display_width/8);   // * blanking_height / 8);
  if (!gpFrameBuf)
  {
    perror("malloc error!\n");
    return 1;
...
void display_buffer(uint8_t* addr, uint32_t x, uint32_t y, uint32_t w, uint32_t h) {
    ...
    IT8951Load1bppImage(addr, x, y, w, h);
    if(IT8951_verbose)
      Serial.println("Displaying image");
    IT8951DisplayArea1bpp(x, y, w, h, drawMode, 0, 0xff);  //note change to 1bpp
    if(IT8951_verbose)
...
void display_clear() {
  if(!gpFrameBuf)
    return;
  for(int i = 0; i < blanking_areas; i++)
    display_buffer(gpFrameBuf, 0, blanking_height*i, display_width, blanking_height);  
}
